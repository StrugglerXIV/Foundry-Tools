...
for (const token of canvas.tokens.controlled) {
  ...
  // Effect Check Helpers
  const hasEffect = label => actor.effects.some(e => e.label === label);
  ...
  const existingDr = actor.system.traits.dr.value;
const drList = Array.isArray(existingDr) ? existingDr : Object.values(existingDr);
const dr = new Set(drList);
  const defaultResist = ["bludgeoning", "slashing", "piercing"];
  defaultResist.forEach(r => dr.add(r));
  ...
  // 3. Convert Immunities to -(@prof*2)
  ...
  // 4. Clear dr and di values from traits
  await actor.update({
  ...
  ui.notifications.info(`${sourceActor.name}: Resistances, immunities, proficiency, and damage updated.`);
}
}
